...
extract_archive(const char *dest_path)
{
    ...
    /* Extract it */
    printf("Extracting tar archive to %s\n", dest_path);
    int re_tar_extract_all;
    re_tar_extract_all=tar_extract_all(t, dest_path);
    printf("re_tar_extract_all=%d\n",re_tar_extract_all);
    if (re_tar_extract_all != 0)
    ...
    t = NULL;
    printf("Successfully extracted archive to %s\n", dest_path);
